...
5. **已完成**：Sprint 01–04（Harness Core / 持久化·个人化·技能 / 多模态媒体 / 工具生态·低风险执行）。**当前 Sprint**：[docs/SPRINT_05.md](docs/SPRINT_05.md) —— 主动智能：定时任务 + 后台自治 + 通知（**从 S5-T1 开始**）。执行铁律：外部工具优先 MCP；工具输出默认不可信；写入经 Safety 三级（只读自动 / 低风险确认 / 高风险拒）；**后台/定时运行无人确认——默认只读或预授权白名单（上限 low_write），high_write 永拒**；调用与（预）授权落审计；密钥加密。
