...
5. **已完成**：Sprint 01–05（Harness Core / 持久化·个人化·技能 / 多模态媒体 / 工具生态·低风险执行 / 主动智能：定时+后台 Job+通知）。执行铁律：外部工具优先 MCP；工具输出默认不可信；写入经 Safety 三级（只读自动 / 低风险确认 / 高风险拒）；**后台/定时运行无人确认——默认只读或预授权白名单（上限 low_write），high_write 永拒；配额计入后台**；调用与（预）授权落审计；密钥加密。后续候选见 [SPRINT_05](docs/SPRINT_05.md) §0。
