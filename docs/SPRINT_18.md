...
- [ ] `McpServer` 协议核心（initialize/tools/list/tools/call + JSON-RPC error）+ `CapabilityTool` 注册表。
- [ ] Apolla 能力暴露为 MCP 工具（研究/翻译/总结/技能/工作区读），owner-scoped、只读/低风险。
- [ ] `POST /api/mcp`（JSON-RPC + API token 鉴权），与 S11 `HttpMCPClient` 互通。
- [ ] 治理：owner 隔离 + 配额 + 限流 + 审计 + 追踪；高风险不暴露；入参 zod 校验。
- [ ] 连接面板（端点 + token 用法 + 客户端配置片段 + 工具列表）。
- [ ] Dogfood：`HttpMCPClient` ↔ Apolla MCP Server 对环通过。
- [ ] `pnpm test` + `pnpm e2e` 全绿且 hermetic。
- [ ] README/架构文档更新。
