...
**账号安全 / MFA + 无密码（Sprint 20）**：把身份故事补全成 **密码（S10）→ OAuth（S14）→ 第二因子 + 无密码（S20）**。**MFA（TOTP）**：设置页开启(扫 `otpauth` 二维码 + 验码确认 + 一次性**备份码**)；启用后**登录 step-up**——密码正确**不等于**登录,先发短时签名 pending 凭证,验过 TOTP 或单次备份码才发会话(**fail-closed**:pending 态无任何受保护访问)。**无密码魔法链接**:"邮件登录链接" → 签名、**单次、限时**的链接(离线 Stub 投递)→ 点开免密登录;请求**恒 200**(账号枚举安全)。TOTP secret 经 `SECRETS_KEY` 加密落库、备份码 scrypt 哈希、token 签名单次——三者绝不入日志/响应。时钟可注入(测试确定性)、限流 + 审计;真实邮件投递是部署事项(Stub 默认)。

**语音 / Speech I/O（Sprint 19）**：语音走可插拔 **SpeechProvider**——默认 **Stub**（离线确定性、`synthesize→transcribe` 可往返），配 `OPENAI_API_KEY` 切到 **OpenAI**（Whisper ASR + TTS，fetch-based）。研究页有**麦克风按钮**（`MediaRecorder` 录音 → `POST /api/speech/transcribe` → **回填问题输入框**，由你审阅后再点研究——**转写是不可信数据，绝不自动提交/执行**）+ 报告**朗读**（`POST /api/speech/synthesize` → 音频落对象存储经 `/media/<key>` 播放）。端到端:**说问题 → 转写 → 研究 → 朗读答案**。owner-scoped + 限流 + 审计 + 追踪 + 大小/长度上限;音频不入日志;无麦克风时优雅降级。

**开放能力供给 / Apolla 作为 MCP Server（Sprint 18）**：S11 让 Apolla 用别人的工具，本 Sprint 让 Apolla**被别人用**——把研究/翻译/总结/技能/工作区读以 **MCP 工具**暴露在 `POST /api/mcp`（JSON-RPC，与 S11 `HttpMCPClient` 互通）。任意 MCP 客户端（Claude Desktop / Cursor / IDE）用一个 **API token** 即可连接：`tools/list` 看到 `apolla.research`/`apolla.translate`/…，`tools/call` **owner-scoped** 执行并拿结果。全程过**配额 + 限流 + 审计 + 追踪**，且**只暴露只读/低风险**能力（写/扣费/破坏/自治永不经 MCP）；入参 zod 校验。设置页有"MCP server"面板（端点 + 工具目录 + 可粘贴的客户端配置）。Apolla 自己的 `HttpMCPClient` 连自己的 server 对环自测（协议真互通）。

**可观测性 / 分布式追踪（Sprint 17）**：可观测性走可插拔 **Tracer**——默认 **Noop**（零开销、离线确定性），配 `OTEL_EXPORTER_OTLP_ENDPOINT` 切到 **OpenTelemetry**（OTLP/HTTP 导出，`OTEL_SERVICE_NAME`/`OTEL_TRACES_SAMPLER_ARG` 调服务名/采样）。span 贯穿 HTTP 请求 → 编排（研究 plan/generate）→ LLM Router(`llm.complete`) → Job；**跨进程链路追踪**:入队时把 traceparent 写入 `Job` → Worker 续上，使 **Web→队列→Worker 成为一条 trace**;request-id ↔ trace-id 关联。`/metrics` 增 **per-operation SLO**(http/job:&lt;kind&gt; 的计数/错误率/p50·p95);`/api/health` 报告 jobQueue 与 tracing 模式;Web 与 Worker 关停前 flush span。**span 绝不含密钥/PII**(属性脱敏、owner 哈希);**入站 traceparent 不可信**(仅关联,绝不参与鉴权)。本地起 collector(Jaeger/Tempo 等 OTLP 兼容)即可看 trace;不配则零开销。
...
**身份 / OAuth 登录（Sprint 14）**：登录走可插拔 **AuthProvider**——默认 **Stub**（离线确定性，授权即回调，便于本地/CI 演示），配 `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`、`GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET` 时注册 **Google / GitHub**（fetch-based、无 SDK；缺 key 不注册）。登录页只展示后端实际注册的 provider。流程：`/api/auth/oauth/:provider/start`（state + PKCE → 302 到授权页）→ `/callback`（**验 state** → 换 code → 取 identity → **按已验证邮箱归一账号** → 复用签名会话）。**账号归一**：同邮箱的密码账号与多个 OAuth 身份属于同一用户（`me.identities` 可见，设置页「已连账号」）。安全：state **单次 + 过期 + 绑 PKCE**、回跳**开放重定向 allowlist**（仅安全相对路径）、`emailVerified=false` **fail-closed 拒**、**OAuth token 不落库**（只存 providerId + email）、登录落审计。真实 provider 需在其控制台配置回调 `…/api/auth/oauth/:provider/callback`。stub 模式可离线端到端演示。
