...
- [x] `Tracer`/`Span` 升级（子 span/status/events/inject·extract/shutdown，向后兼容）+ `InMemoryTracer` + 脱敏。
- [x] `OtelTracer`（OTLP，env 门控，缺 endpoint → Noop）。
- [x] 埋点：研究（plan/generate）+ Router（`llm.complete`）+ Job；属性脱敏。（Cowork 子代理/工具更深的子 span 经 AGENT/LLM 嵌套覆盖；更细粒度留作后续。）
- [x] HTTP + Job span + **跨进程透传**（traceparent 经 `Job`：Web→Worker 一条 trace）；request-id ↔ trace-id。
- [x] 指标/SLO 增强经 `/metrics`（per-operation 计数/错误率/p50·p95）；采样；关停 flush（Web + Worker）。
- [x] 安全：span 无密钥/PII；入站 traceparent 不可信；追踪不改行为/不阻塞（Noop 零开销）。
- [x] `pnpm test` + `pnpm e2e` 全绿且 hermetic（Noop/InMemory，不连 collector）。
- [x] README/架构文档更新。

> **Sprint 17 完成**（PR [#96](https://github.com/Timsunzhuping/ApollaAIStudio/pull/96) A · [#97](https://github.com/Timsunzhuping/ApollaAIStudio/pull/97) B · [#98](https://github.com/Timsunzhuping/ApollaAIStudio/pull/98) C · D 本次）。可换挡 **Tracer**（Noop 默认 / `@apolla/otel` 的 OtelTracer，OTLP env 门控）+ `InMemoryTracer` + 升级的 `Tracer`/`Span`（子 span/inject·extract/shutdown）+ `AsyncLocalStorage` span scope（`traced`/`tracedGen`）。埋点：`http.request` → 编排（research plan/generate）→ `llm.complete` → `job.run`，子 span 自动嵌套。**跨进程链路追踪**：`Job.traceparent` 在入队时自动捕获、Worker 续上 → Web→队列→Worker 一条 trace。`/metrics` per-operation SLO（http/job:&lt;kind&gt;）；`/api/health` 报告 jobQueue/tracing；关停 flush。span 脱敏（无密钥/PII、owner 哈希）；入站 traceparent 不可信。默认 Noop 零开销、全程 hermetic：285 root + 8 skip + eval 39 + 9 e2e 绿。新增 eval `tracing-propagation`。OTel 自动埋点/RUM/告警/profiling 列为后续。
