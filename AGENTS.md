...
5. **已完成**：Sprint 01–04（Harness Core / 持久化·个人化·技能 / 多模态媒体 / 工具生态·低风险执行）。执行铁律：外部工具优先 MCP；工具输出默认不可信（数据通道）；写入动作必经 Safety 三级（只读自动 / 低风险需确认 / 高风险拒绝）；每次调用与确认落审计；连接器密钥加密。后续候选见 [SPRINT_04](docs/SPRINT_04.md) §0。
