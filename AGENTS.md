...
5. **当前 Sprint**：[docs/SPRINT_01.md](docs/SPRINT_01.md) —— 从空仓库到 Harness Core 骨架 + 第一条「研究→成品」闭环的逐任务执行单。**从 T1 开始。**
...
## 命令
> 阶段0 搭建 monorepo 后补全：`build` / `test` / `lint` / `dev` / `eval` / `contract-test`。
