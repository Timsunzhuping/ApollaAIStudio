...
- [ ] TOTP（RFC 6238，注入时钟）+ 备份码（scrypt 哈希 + 单次）+ `otpauth` URI + `MfaEnrollment` 契约。
- [ ] 魔法链接（签名 + 单次 + 限时 token + 投递）+ 单次态仓库（内存 + Postgres）。
- [ ] MFA 注册/验证/关闭 + **登录 step-up**（启用后仅密码拿不到完整会话）；`users` MFA 列（secret 加密、备份码哈希）。
- [ ] 魔法链接 请求（恒 200）/ 校验（→会话）端点，限流 + 审计 + 枚举安全。
- [ ] Web：设置页 MFA + 登录 MFA 挑战 + 无密码入口/落地。
- [ ] 安全：step-up fail-closed、secret 加密、备份码/token 哈希·签名单次、不入日志。
- [ ] `pnpm test` + `pnpm test:web` + `pnpm e2e` 全绿且 hermetic（注入时钟 + Stub 投递）。
- [ ] README/架构文档更新。
