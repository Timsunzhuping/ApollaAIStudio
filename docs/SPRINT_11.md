...
- [x] HTTP/SSE MCP 客户端：JSON-RPC over HTTP（JSON + SSE 响应）、initialize/list/call/close、超时不挂起；内置 stub server 契约测试通过。
- [x] 远程连接器：transport='http' 用 url + 加密 token（仅发配置 host）；工具枚举 + 保守风险；不可达隔离不崩；远程调用计限流 + 审计。
- [x] 市场：声明式目录 + 一键添加；新增条目零业务代码改动；缺 secret 报清晰错。
- [x] 市场 UI：浏览目录 → 添加 → 工具可见 → Agent/Cowork 调用。
- [x] 安全：远程输出 untrusted 不升级、远程不自动 high_write、secrets 不入日志/响应、远程调用受限流。
- [x] 健康/可观测：连接器健康端点 + 指标 + UI 徽标；失败隔离。
- [x] `pnpm test` + `pnpm test:web` 覆盖客户端契约/远程端到端/安全/健康/市场；CI 全门禁绿。
- [x] README/架构文档更新；Demo 离线可演示（内置 stub server）。

> **Sprint 11 完成。** S11-T1–T8 全部合并到 main（PR #66–#68 + 本 PR）。HTTP/SSE MCP transport（`HttpMCPClient` + `StubHttpMcpServer`）打通托管 MCP 生态；连接器市场（目录 + 一键添加 + UI）；远程工具安全（token 加密只发配置 host、输出 untrusted、绝不自动 high_write、不可达隔离）+ 连接器健康探针/指标/徽标。新增 1 项 eval（remote-http-tool 端到端）→ 35 项；root 237 + web 17 测试；CI 全门禁绿。复用既有 MCP/Tool/Safety 抽象，不另造执行通道。
